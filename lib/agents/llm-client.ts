...
export class LLMClient {
  private getApiKey(): string {
    ...
    if (!apiKey) {
      throw new Error("OPENAI_API_KEY 环境变量未设置。请在 .env.local 文件中设置此变量。")
    }
    return apiKey
  }
  ...
  async processWithTools(
    ...
    try {
      ...
      if (error.message?.includes("API key") || error.message?.includes("OPENAI_API_KEY")) {
        throw new Error("OpenAI API Key 未设置或无效。请检查 .env.local 文件中的 OPENAI_API_KEY 设置。")
      }

      if (error.message?.includes("401")) {
        throw new Error("OpenAI API Key 无效。请检查您的 API Key 是否正确。")
      }

      if (error.message?.includes("quota")) {
        throw new Error("OpenAI API 配额已用完。请检查您的账户余额。")
      }
